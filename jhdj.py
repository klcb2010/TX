...
class Spider(Spider):
    ...
    def searchContent(self, key, quick, pg="1"):
        ...
        wd_lower = key.lower()
        # 提取关键词部分（用于模糊匹配）
        key_parts = [k for k in ['逍遥', '无敌', '战神', '神医', '赘婿', '总裁', '萌宝', '穿越', '重生', '逆袭'] if k in wd_lower]
        if not key_parts:
            key_parts = [wd_lower]  # 如果没有匹配到常见关键词，使用完整关键词

        def safe_push(item, require_match=True, fuzzy_match=False):
            if not item or not item.get('vod_id') or not item.get('vod_name'):
                return
            name_lower = item['vod_name'].lower()
            # 精确匹配：标题包含完整搜索词
            if require_match and wd_lower and wd_lower not in name_lower:
                # 如果精确匹配失败，检查是否包含关键词部分（模糊匹配）
                if fuzzy_match:
                    if not any(part in name_lower for part in key_parts):
                        return
                else:
                    return
        ...
        # 七猫搜索 - 搜索API已失效，改用首页API + 关键词过滤
        try:
            sign = self._md5(f"operation=1playlet_privacy=1tag_id=0{self.keys}")
            url = f"{self.platform['七猫']['host']}{self.platform['七猫']['url1']}?tag_id=0&playlet_privacy=1&operation=1&sign={sign}"
            header_x = self._get_header_x()
            res = self._request(url, headers={**header_x, **self.headers_default}, timeout=6000)
            for i in res.get('data', {}).get('list', []):
                safe_push({
                    'vod_id': f"七猫@{quote(str(i['playlet_id']))}",
                    'vod_name': i['title'],
                    'vod_pic': i['image_link'],
                    'vod_remarks': f"七猫短剧｜{i['total_episode_num']}集"
                }, require_match=True, fuzzy_match=True)
        except:
            pass
        ...
        try:
            plat = self.platform['星芽']
            headers = self._ensure_xingya_auth()
            res = self._request(plat['host'] + plat['search'], method='POST', headers=headers, data={'text': key}, timeout=10000)
            for i in res.get('data', {}).get('theater', {}).get('search_data', []):
                safe_push({
                    'vod_id': f"星芽@{plat['host']}{plat['url2']}?theater_parent_id={i['id']}",
                    'vod_name': i['title'],
                    'vod_pic': i['cover_url'],
                    'vod_remarks': f"星芽短剧｜{i.get('total', '')}集 播放:{i.get('play_amount_str', '')}"
                }, require_match=True, fuzzy_match=True)
        except:
            pass

        # 西饭搜索 - 修复：正确解析 elements[].contents[].duanjuVo
        try:
            plat = self.platform['西饭']
            url = f"{plat['host']}{plat['search']}?reqType=search&offset={(pg - 1) * 30}&keyword={quote(key)}&quickEngineVersion=-1&scene="
            res = self._request(url)
            for block in res.get('result', {}).get('elements', []):
                for item in block.get('contents', []):
                    dj = item.get('duanjuVo') or {}
                    if not dj.get('duanjuId') or not dj.get('title'):
                        continue
                    safe_push({
                        'vod_id': f"西饭@{dj['duanjuId']}#{dj.get('source', '')}",
                        'vod_name': dj['title'],
        ...
        # 围观搜索 - 暂时禁用，因为API搜索功能无效（返回固定推荐列表）
        # try:
        ...
        #     client_info = self._md5(str(int(time.time() * 1000))[-10:])
        #     url = f"{plat['host']}{plat['search']}?version_code=1500&version_name=1.5.0&device_name={quote(device_name)}&device_type=phone&is_first_day=true&is_first_24h=true&app_launch_way=icon&default_homepage=homepage_interaction&device_owning_firm={quote(device_firm)}&font_scale=default&os_type=1&clientInfo={client_info}"
        #     res = self._request(url, method='POST', headers={'User-Agent': 'okhttp/5.1.0', 'Content-Type': 'application/json; charset=utf-8'}, data={'audience': '', 'order': '', 'page': pg, 'pageSize': 30, 'searchWord': key, 'subject': ''}, timeout=10000)
        #     for i in res.get('data', []):
        #         safe_push({
        #             'vod_id': f"围观@{i['oneId']}",
        #             'vod_name': i['title'],
        ...
        try:
            ...
            res = self._request(f"{plat['host']}{plat['search']}", method='POST', headers=headers, data={'sourceType': 1, 'keyword': key, 'index': pg}, timeout=10000)
