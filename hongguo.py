...
_RANK_FOLDER_PIC = "data:image/png;base64," + "".join(
    ...
    ZZneh6GD/fkAxCIBYBu69nv5BxbkV3ufcqcBy3o/GEY39v/59ERDQAHgVIeD2gOYHlQTSKxvBr7Rkf3/E9u/KYDqnExrAPdA
    ...
    try:
        ...
        for src in sources:
            try:
                ...
                continue
        # 只保留 1080P：详情页不再展开成 5 档，单线直接命名为 1080P。
        try:
...
class Spider(_BaseSpider):
    ...
    def playerContent(self, flag, id, vipFlags):
        try:
            ...
            # 只保留 1080P：固定取 1080P，取不到就回退第一条。
            best = options[0] if isinstance(options[0], dict) else {}
            try:
                for opt in options:
                    if isinstance(opt, dict) and str(opt.get("name") or "").strip().upper() == "1080P":
                        best = opt
                        break
            except Exception:
                pass
